...
- DeepSeek thinking/reasoning 可选开关（`config.json` 设 `"thinking": true`）
...
| `upstream` | `https://api.deepseek.com` | API 地址 |
| `thinking` | `false` | 开启 DeepSeek reasoning（思考过程），`true` 时响应含 reasoning output item |
